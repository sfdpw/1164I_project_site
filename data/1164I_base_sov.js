var base_sov = 

[
 {
  'bid_item': 'G-1',
  'worksheet': 1,
  'description': 'TEMPORARY RETROFLECTIVE PAVEMENT MARKINGS',
  'unit': 'LF',
  'contract_qty': 2400,
  'unit_price':1,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'G-01', 1)
  },

 {
  'bid_item': 'G-2',
  'worksheet': 0,
  'description': 'ALLOWANCE FOR PARTNERING FACILITATION AND RELATED COSTS',
  'unit': 'AL',
  'contract_qty': 1,
  'unit_price':15000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'G-3',
  'worksheet': 0,
  'description': 'ALLOWANCE FOR AS- NEEDED TRANSIT SUPPORT PROVIDED BY CONTRACTOR',
  'unit': 'AL',
  'contract_qty': 1,
  'unit_price':1040,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'G-4',
  'worksheet': 0,
  'description': 'ALLOWANCE FOR AS- NEEDED OVERHEAD CONTRACT SYSTEM (OCS) SUPPORT PROVIDED BY CONTRACTOR',
  'unit': 'AL',
  'contract_qty': 1,
  'unit_price':7500,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'R-1',
  'worksheet': 0,
  'description': 'TRAFFIC CONTROL FOR ROADWAY  WORK',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':247000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':12967.50,   
                                   'PP05':53722.50,    
                                   'PP06':22230,                               
                                   'PP07':7410, 
                                   'PP08':37050,                               
                                   'PP09':9880,      
				   'PP10':4940,
				   'PP11':37050,
				   'PP12':0.00,
				   'PP13':49400,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'R-2',
  'worksheet': 1,
  'description': 'FULL DEPTH PLANING PER 2-INCH DEPTH OF CUT',
  'unit': 'SF',
  'contract_qty': 143167,
  'unit_price':0.3,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-02', 0.3)
  },

 {
  'bid_item': 'R-3',
  'worksheet': 1,
  'description': 'HOT MIX ASPHALT (TYPE A, ½-INCH MAXIMUM WITH MEDIUM GRADING)',
  'unit': 'US SHORT TON',
  'contract_qty': 2034,
  'unit_price':190,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-03', 190)
  },

 {
  'bid_item': 'R-4',
  'worksheet': 1,
  'description': '8-INCH THICK CONCRETE BASE',
  'unit': 'SF',
  'contract_qty': 24021,
  'unit_price': 14,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-04', 14)
  },

 {
  'bid_item': 'R-5',
  'worksheet': 2,
  'description': '10-INCH THICK CONCRETE BASE',
  'unit': 'SF',
  'contract_qty': 6235,
  'unit_price':15,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-05', 15)
  },

 {
  'bid_item': 'R-6',
  'worksheet': 1,
  'description': '8-INCH THICK CONCRETE PAVEMENT, PARKING STRIP, OR GUTTER',
  'unit': 'SF',
  'contract_qty': 82935,
  'unit_price':15,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-06', 15)
  },

 {
  'bid_item': 'R-7',
  'worksheet': 1,
  'description': '10-INCH THICK CONCRETE BUS PAD (6,000 PSI)',
  'unit': 'SF',
  'contract_qty': 2185,
  'unit_price':17,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-07', 17)
  },

 {
  'bid_item': 'R-8',
  'worksheet': 1,
  'description': 'AGGREGATE BASE CLASS II FOR BUS PAD',
  'unit': 'CY',
  'contract_qty': 81,
  'unit_price':250,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-08', 250)
  },

 {
  'bid_item': 'R-9',
  'worksheet': 1,
  'description': '3 ½-INCH THICK CONCRETE SIDEWALK',
  'unit': 'SF',
  'contract_qty': 8500,
  'unit_price':13,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-09', 13)
  },

 {
  'bid_item': 'R-10',
  'worksheet': 2,
  'description': '6-INCH WIDE CONCRETE CURB',
  'unit': 'LF',
  'contract_qty': 436,
  'unit_price':70,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-10', 70)
  },

 {
  'bid_item': 'R-11',
  'worksheet': 2,
  'description': '4-INCH WIDE CONCRETE CURB',
  'unit': 'LF',
  'contract_qty': 86,
  'unit_price':50,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-11', 50)
  },

 {
  'bid_item': 'R-12',
  'worksheet': 2,
  'description': 'RESET EXISTING GRANITE CURB',
  'unit': 'LF',
  'contract_qty': 94,
  'unit_price':115,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-12', 115)
  },

 {
  'bid_item': 'R-12A',
  'worksheet': 2,
  'description': '6-INCH WIDE PRECAST GRANITE CURB',
  'unit': 'LF',
  'contract_qty': 45,
  'unit_price':115,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-12A', 115)
  },

 {
  'bid_item': 'R-13',
  'worksheet': 2,
  'description': 'COMBINED 6-INCH WIDE CONCRETE CURB AND 2-FOOT WIDE CONCRETE GUTTER',
  'unit': 'LF',
  'contract_qty': 1201,
  'unit_price':81,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-13', 81)
  },

 {
  'bid_item': 'R-14',
  'worksheet': 2,
  'description': 'CONCRETE CURB RAMP WITH CONCRETE DETECTABLE SURFACE TILES',
  'unit': 'EA',
  'contract_qty': 36,
  'unit_price':3200,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-14', 3200)
  },

 {
  'bid_item': 'R-15',
  'worksheet': 2,
  'description': 'PULL BOX REPLACEMENT TYPE I (CONTINGENCY BID ITEM)',
  'unit': 'EA',
  'contract_qty': 10,
  'unit_price':120,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-15', 120)
  },

 {
  'bid_item': 'R-16',
  'worksheet': 2,
  'description': 'CONSTRUCT AWSS FIRE CISTERN IDENTIFICATION PAVEMENT BRICK RING',
  'unit': 'EA',
  'contract_qty': 1,
  'unit_price':11750,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-16', 11750)
  },

 {
  'bid_item': 'R-17',
  'worksheet': 2,
  'description': 'HMA SPEED HUMP / CUSHION',
  'unit': 'EA',
  'contract_qty': 1,
  'unit_price':3500,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'R-17', 3500)
  },

 {
  'bid_item': 'R-18',
  'worksheet': 0,
  'description': '**ADJUST CITY-OWNED MANHOLE FRAME AND COVER AND CATCH BASIN FRAME AND GRATE TO GRADE (CONTINGENCY  BID ITEM)',
  'unit': 'EA',
  'contract_qty': 42,
  'unit_price':50,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'R-19',
  'worksheet': 0,
  'description': '**ADJUST CITY-OWNED HYDRANT AND WATER MAIN VALVE BOX CASTING COVER TO GRADE (CONTINGENCY BID ITEM)',
  'unit': 'EA',
  'contract_qty': 100,
  'unit_price':50,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'R-20',
  'worksheet': 0,
  'description': 'MOBILIZATION FOR ROADWAR WORK (MAXIMUM 3% OF SUM OF BID ITEMS R-1 THROUGH R-19)',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':62000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':31000,   
                                   'PP05':31000,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'R-21',
  'worksheet': 0,
  'description': 'DEMOBILIZATION FOR ROADWAY WORK (MAXIMUM 2% OF SUM OF BID ITEMS R-1 THROUGH R-19)',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':13000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': ' SW-1',
  'worksheet': 0,
  'description': 'TRAFFIC ROUTING FOR SEWER WORK',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':150000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':5400,
                                   'PP02':19350,
                                   'PP03':22500,    
                                   'PP04':0.00,   
                                   'PP05':12750,    
                                   'PP06':1500,                               
                                   'PP07':30000, 
                                   'PP08':28500,                               
                                   'PP09':12000,      
                                   'PP10':13000,
				   'PP11':0.00,
				   'PP12':5000,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-2',
  'worksheet': 0,
  'description': 'TRENCH AND EXCAVATION  SUPPORT FOR SEWER WORK',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':290000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':11890,
                                   'PP02':43210,
                                   'PP03':49300,    
                                   'PP04':0.00,   
                                   'PP05':7250,    
                                   'PP06':4350,                               
                                   'PP07':66700, 
                                   'PP08':63800,                               
                                   'PP09':14500,      
                                   'PP10':24000,
				   'PP11':0.00,
				   'PP12':5000,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-3',
  'worksheet': 1,
  'description': 'STANDARD CONCRETE MANHOLE FOR 12-INCH TO 24-INCH DIAMETER SEWERS WITH FRAME AND COVER (PER SFDPW STANDARD PLAN 87,181)',
  'unit': 'EA',
  'contract_qty': 34,
  'unit_price':6000,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_points_1'], 'SW-03', 6000)
  },

 {
  'bid_item': 'SW-4',
  'worksheet': 1,
  'description': 'CLEAN AND MORTAR EXISTING MANHOLE',
  'unit': 'EA',
  'contract_qty': 12,
  'unit_price':5050,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_points_1'], 'SW-04', 5050)
  },

 {
  'bid_item': 'SW-5',
  'worksheet': 1,
  'description': '12-INCH DIAMETER VCP SEWER ON CRUSHED ROCK BEDDING WRAPPED IN GEOTEXTILE  FABRIC',
  'unit': 'LF',
  'contract_qty': 3086,
  'unit_price':310.5,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-05', 310.5)
  },

 {
  'bid_item': 'SW-6',
  'worksheet': 1,
  'description': '15-INCH DIAMETER VCP SEWER ON CRUSHED ROCK BEDDING WRAPPED IN GEOTEXTILE  FABRIC',
  'unit': 'LF',
  'contract_qty': 123,
  'unit_price':250,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-06', 250)
  },

 {
  'bid_item': 'SW-7',
  'worksheet': 1,
  'description': '18-INCH DIAMETER VCP SEWER ON CRUSHED ROCK BEDDING WRAPPED IN GEOTEXTILE  FABRIC',
  'unit': 'LF',
  'contract_qty': 206,
  'unit_price':390,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-07', 390)
  },

 {
  'bid_item': 'SW-8',
  'worksheet': 1,
  'description': '21-INCH DIAMETER VCP SEWER ON CRUSHED ROCK BEDDING WRAPPED IN GEOTEXTILE  FABRIC',
  'unit': 'LF',
  'contract_qty': 213,
  'unit_price':440,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-08', 440)
  },

 {
  'bid_item': 'SW-9',
  'worksheet': 1,
  'description': 'LINE EXISTING 8-INCH INSIDE DIAMETER PIPE WITH STYRENE-FREE CURED-IN-PLACE LINER (CIPL)',
  'unit': 'LF',
  'contract_qty': 56,
  'unit_price':545,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-09', 545)
  },

 {
  'bid_item': 'SW-10',
  'worksheet': 2,
  'description': 'LINE EXISTING 10-INCH INSIDE DIAMETER PIPE WITH STYRENE-FREE CURED-IN-PLACE LINER (CIPL)',
  'unit': 'LF',
  'contract_qty': 100,
  'unit_price':252,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-10', 252)
  },

 {
  'bid_item': 'SW-11',
  'worksheet': 2,
  'description': 'LINE EXISTING 12-INCH INSIDE DIAMETER PIPE WITH STYRENE-FREE CURED-IN-PLACE LINER (CIPL)',
  'unit': 'LF',
  'contract_qty': 95,
  'unit_price':283,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-11', 283)
  },

 {
  'bid_item': 'SW-12',
  'worksheet': 2,
  'description': 'LINE EXISTING 15-INCH INSIDE DIAMETER PIPE WITH STYRENE-FREE CURED-IN-PLACE LINER (CIPL)',
  'unit': 'LF',
  'contract_qty': 264,
  'unit_price':136,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-12', 136)
  },

 {
  'bid_item': 'SW-13',
  'worksheet': 2,
  'description': 'LINE EXISTING 21-INCH INSIDE DIAMETER PIPE WITH STYRENE-FREE CURED-IN-PLACE LINER (CIPL)',
  'unit': 'LF',
  'contract_qty': 202,
  'unit_price':252,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-13', 252)
  },

 {
  'bid_item': 'SW-14',
  'worksheet': 0,
  'description': 'CLEAN AND TELEVISE EXISTING MAIN SEWER PRIOR TO LINING WITH CIPL',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':11000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':11000,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-15',
  'worksheet': 2,
  'description': 'REMOVE OBSTRUCTION REQUIRING INTERNAL REAMING FROM EXISTING SEWER PRIOR TO LINING WORK',
  'unit': 'EA',
  'contract_qty': 8,
  'unit_price':1000,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-15', 1000)
  },

 {
  'bid_item': 'SW-16',
  'worksheet': 0,
  'description': 'BYPASSING MAIN SEWER FLOW RELATED TO LINING',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':1000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-17',
  'worksheet': 2,
  'description': '**BYPASSING SIDE SEWER FLOW RELATED TO LINING (CONTINGENCY BID ITEM)',
  'unit': 'EA',
  'contract_qty': 27,
  'unit_price':200,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-17', 200)
  },

 {
  'bid_item': 'SW-18',
  'worksheet': 0,
  'description': '**INTERNALLY  REINSTATE CONNECTIONS  IN NEWLY LINED MAIN SEWERS (CONTINGENCY BID ITEM)',
  'unit': 'EA',
  'contract_qty': 27,
  'unit_price':50,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-19',
  'worksheet': 0,
  'description': '**PLUG AND FILL EXISTING SEWERS WITH SLURRY GROUT (CONTINGENCY BID ITEM)',
  'unit': 'CY',
  'contract_qty': 5,
  'unit_price':200,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-20',
  'worksheet': 2,
  'description': '**6-INCH OR 8-INCH DIAMETER SIDE SEWER CONNECTION TO VCP MAIN SEWER (CONTINGENCY BID ITEM)',
  'unit': 'EA',
  'contract_qty': 235,
  'unit_price':180,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-20', 180)
  },

 {
  'bid_item': 'SW-21',
  'worksheet': 2,
  'description': '**TELEVISION  INSPECTION OF EXISTING 6-INCH OR 8- INCH DIAMETER SIDE SEWER AND 10-INCH DIAMETER CULVERT LOCATED WITHIN LIMITS OF MAIN SEWER WORK (CONTINGENCY BID ITEM)',
  'unit': 'EA',
  'contract_qty': 134,
  'unit_price':12,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-21', 12)
  },

 {
  'bid_item': 'SW-22',
  'worksheet': 0,
  'description': '**TELEVISION INSPECTION OF EXISTING 6-INCH OR 8- INCH DIAMETER SIDE SEWER AND 10-INCH DIAMETER CULVERT LOCATED OUTSIDE THE LIMITS OF MAIN SEWER WORK BUT WITHIN CONTRACT LIMITS (CONTINGENCY  BID ITEM)',
  'unit': 'EA',
  'contract_qty': 10,
  'unit_price':12,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-23',
  'worksheet': 2,
  'description': '**CONSTRUCTION  OR REPLACEMENT  OF 6- INCH OR 8-INCH DIAMETER  SIDE SEWER LOCATED WITHIN LIMITS OF MAIN SEWER WORK (CONTINGENCY BID ITEM)',
  'unit': 'LF',
  'contract_qty': 2500,
  'unit_price':100,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-23', 100)
  },

 {
  'bid_item': 'SW-24',
  'worksheet': 2,
  'description': '**CONSTRUCTION  OR REPLACEMENT  OF 6 OR 8-INCH DIAMETER SIDE SEWER LOCATED OUTSIDE THE LIMITS OF MAIN SEWER WORK BUT WITHIN CONTRACT LIMITS (CONTINGENCY BID ITEM)',
  'unit': 'LF',
  'contract_qty': 200,
  'unit_price':120,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-24', 120)
  },

 {
  'bid_item': 'SW-25',
  'worksheet': 2,
  'description': '**10-INCH DIAMETER CULVERT (CONTINGENCY  BID ITEM)',
  'unit': 'LF',
  'contract_qty': 242,
  'unit_price':200,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-25', 200)
  },

 {
  'bid_item': 'SW-26',
  'worksheet': 2,
  'description': 'CONCRETE CATCH BASIN WITHOUT CURB INLET AND WITH NEW FRAME AND GRATING (PER SFDPW STANDARD PLAN 87,188)',
  'unit': 'EA',
  'contract_qty': 3,
  'unit_price':7000,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_points_1'], 'SW-26', 7000)
  },

 {
  'bid_item': 'SW-27',
  'worksheet': 0,
  'description': '**CAST IRON WATER TRAP FOR EXISTING CATCH BASIN INCLUDING CLEANOUT CAP (PER SFDPW STANDARD PLAN 87,194) (CONTINGENCY  BID ITEM)',
  'unit': 'EA',
  'contract_qty': 32,
  'unit_price':350,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-28',
  'worksheet': 2,
  'description': 'POST-CONSTRUCTION TELEVISION INSPECTION OF NEWLY CONSTRUCTED MAINS AND LINED MAIN SEWERS',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':12000,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-28', 12000)
  },

 {
  'bid_item': 'SW-29',
  'worksheet': 2,
  'description': '**POST-CONSTRUCTION TELEVISION INSPECTION OF NEWLY CONSTRUCTED SIDE SEWERS AND CULVERTS (CONTINGENCY  BID ITEM)',
  'unit': 'EA',
  'contract_qty': 167,
  'unit_price':12,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_101_sewer_lines_0'], 'SW-29', 12)
  },

 {
  'bid_item': 'SW-30',
  'worksheet': 2,
  'description': '**RECONSTRUCT PAVEMENT OUTSIDE OF SEWER T-TRENCH LIMIT AND OUTSIDE OF CONCRETE BASE WORK UNDER R-DRAWINGS WITH 8-INCH THICK CONCRETE BASE NECESSARY  PER EXCAVATION  CODE AS DIRECTED BY THE CITY REPRESENTATIVE (CONTINGENCY  BID ITEM)',
  'unit': 'SF',
  'contract_qty': 6800,
  'unit_price':14,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'SW-30', 14)
  },

 {
  'bid_item': 'SW-31',
  'worksheet': 0,
  'description': '**EXPLORATORY  HOLES OR POTHOLES (CONTINGENCY BID ITEM)',
  'unit': 'CY',
  'contract_qty': 30,
  'unit_price':120,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':2820,      
                                   'PP10':420,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-32',
  'worksheet': 0,
  'description': '**IMPORTED BACKFILL MATERIAL FOR SEWER WORK (CONTINGENCY BID ITEM)',
  'unit': 'CY',
  'contract_qty': 500,
  'unit_price':60,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':4320,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':14400,                               
                                   'PP09':0.00,      
                                   'PP10':8520,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-33',
  'worksheet': 0,
  'description': '**CONTROLLED DENSITY FILL BEDDING MATERIAL FOR  SFWD FACILITIES ENCOUNTERED  WITHIN SEWER TRENCH PIROR TO BACKFILL (CONTINGENCY  BID ITEM)',
  'unit': 'CY',
  'contract_qty': 20,
  'unit_price':165,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-34',
  'worksheet': 0,
  'description': 'DESIGN OF STRUCTURAL  SUPPORT FOR SFWD FACILITIES ENCOUNTERED  WITHIN SEWER TRENCH',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':2000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-35',
  'worksheet': 0,
  'description': 'TRANSPORTATION OF SURPLUS CALIFORNIA CLASS I (NON-RCRA) HAZARDOUS WASTE (SOILS) TO A CLASS I DISPOSAL FACILITY',
  'unit': 'US SHORT TON',
  'contract_qty': 900,
  'unit_price':25,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-36',
  'worksheet': 0,
  'description': 'DISPOSAL OF SURPLUS CALIFORNIA CLASS I (NON-RCRA) HAZARDOUS WASTE (SOILS)  AT A CLASS I DISPOSAL FACILITY',
  'unit': 'US SHORT TON',
  'contract_qty': 900,
  'unit_price':25,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-37',
  'worksheet': 0,
  'description': 'TRANSPORTATION OF SURPLUS NON- HAZARDOUS SOILS (CLASS II & Ill) CALIFORNIA DESIGNATED WASTE (SOILS) TO A CLASS II & Ill  DISPOSAL FACILITY',
  'unit': 'US SHORT TON',
  'contract_qty': 250,
  'unit_price':25,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':5500,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-38',
  'worksheet': 0,
  'description': 'DISPOSAL OF SURPLUS NON-HAZARDOUS  SOILS (CLASS II & Ill) CALIFORNIA DESIGNATED WASTE (SOILS)  AT A CLASS II &  Ill  DISPOSAL FACILITY',
  'unit': 'US SHORT TON',
  'contract_qty': 250,
  'unit_price':25,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-39',
  'worksheet': 0,
  'description': 'UNDISTURBED NATIVE BEDROCK (CONTINGENCY  BID ITEM)',
  'unit': 'CY',
  'contract_qty': 250,
  'unit_price':100,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':7200,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':9900,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-40',
  'worksheet': 0,
  'description': 'MOBILIZATION FOR SEWER AND DRAINAGE WORK (MAXIMUM 3% OF THE SUM OF BID ITEMS SW-1 THROUGH SW-39)',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':60000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':15000,
                                   'PP02':0.00, 
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':45000, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-41',
  'worksheet': 0,
  'description': 'DEMOBILIZATION FOR SEWER AND DRAINAGE WORK (MAXIMUM 2% OF THE SUM OF BID ITEMS SW-1 THROUGH SW-39)',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':20000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-42',
  'worksheet': 0,
  'description': 'ALLOWANCE FOR UNFORSEEN ENVIRONMENTAL WORK',
  'unit': 'AL',
  'contract_qty': 1,
  'unit_price':25000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'SW-43',
  'worksheet': 0,
  'description': 'ALLOWANCE TO PERFORM NECESSARY WORK DUE TO UNFORSEEN CONDITIONS RELATED TO SEWER WORK',
  'unit': 'AL',
  'contract_qty': 1,
  'unit_price':95000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':35150,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':59850,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },
  
 {
  'bid_item': 'CO-01 - PCO-01 - R-09',
  'worksheet': 3,
  'description': '3.5 in Sidewalk at 19 Mirabel Ave',
  'unit': 'SF',
  'contract_qty': 36,
  'unit_price':13,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'CO-01 - PCO-01 - R-09', 13)
  },
  
  {
  'bid_item': 'CO-01 - PCO-01 - R-12',
  'worksheet': 3,
  'description': 'Reset (E) Granite Curb at 19 Mirabel Ave',
  'unit': 'LF',
  'contract_qty': 20,
  'unit_price':115,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'CO-01 - PCO-01 - R-12', 115)
  },
  
  {
  'bid_item': 'CO-01 - PCO-01 - R-19',
  'worksheet': 0,
  'description': 'Adjust (E) Casting at 19 Mirabel Ave',
  'unit': 'EA',
  'contract_qty': 1,
  'unit_price':50,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },
  
  {
  'bid_item': 'CO-01 - PCO-01 - R-23',
  'worksheet': 3,
  'description': 'Reset (E) Brick Pavers at 19 Mirabel Ave',
  'unit': 'SF',
  'contract_qty': 25,
  'unit_price':50,
  'payment_history': return_pp_history_from_spatial_file(['json_1164I_200_flatwork_polygons'], 'CO-01 - PCO-01 - R-23', 50)
  },
  
  {
  'bid_item': 'CO-01 - PCO-01 - R-24',
  'worksheet': 0,
  'description': 'Traffic Control for Flatwork at 19 Mirabel Ave',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price':2000,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':2000,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

  {
  'bid_item': 'CO-02 - PCO-02',
  'worksheet': 0,
  'description': 'CIPL on Mullen Ave',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price': 37909.11,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':37909.11,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  },

 {
  'bid_item': 'CO-02 - PCO-03',
  'worksheet': 0,
  'description': 'Sewer Deletion on Eugenia Ave',
  'unit': 'LS',
  'contract_qty': 1,
  'unit_price': -81993.00,
  'payment_history': {
                      'SFPW-ESH': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':0.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   },
                      'SFPUC-SW': {
                                   'PP01':0.00,
                                   'PP02':0.00,
                                   'PP03':0.00,    
                                   'PP04':0.00,   
                                   'PP05':0.00,    
                                   'PP06':0.00,                               
                                   'PP07':0.00, 
                                   'PP08':0.00,                               
                                   'PP09':0.00,      
                                   'PP10':0.00,
				   'PP11':0.00,
				   'PP12':-81993.00,
				   'PP13':0.00,
				   'PP14':0.00,
				   'PP15':0.00
                                   }
                      }
  }


]
