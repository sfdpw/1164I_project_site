function tracking_table_generator_sw_laterals(asset_type) {
  ...
  var return_block = 
    ...
    '<thead class="track_thead">\
      <tr>\
        ...
        <th class="track_thead" rowspan="2" style="text-align:center">Block Lot</th>\
  ...
  for (const sw_lateral of json_1164I_101_sewer_lines_0["features"])

  { if (sw_lateral.properties.type == 'LT')
   ...
   sw_lateral_row[1] = "<a href=\"..\\index.html#20/" + 
                     ...
                     "\" target=\"_blank\">" +
                     sw_lateral.properties.id_a + "</a>";
   
   sw_lateral_row[2] = sw_lateral.properties.id_b;
   sw_lateral_row[3] = sw_lateral.properties.location;
   ...
   for (var bi in sw_lateral.properties.pp_history)
    ...
    for (bi = 1; bi < sw_lateral_row[6].length; bi++ )
    
      {
       ...
       while (base_sov[NN].bid_item != sw_lateral_row[6][bi].replace('SW-0','SW-')) { NN++; }
   ...
   sw_lateral_row[7] = sw_lateral.properties.submittals.tvi_pre_con.submittal_no;
   sw_lateral_row[8] = sw_lateral.properties.submittals.tvi_pre_con.video_no;
   ...
   sw_lateral_row[10] = sw_lateral.properties.submittals.tvi_post_con.submittal_no;
   sw_lateral_row[11] = sw_lateral.properties.submittals.tvi_post_con.video_no;
   sw_lateral_row[12] = sw_lateral.properties.submittals.tvi_post_con.response;   
  ...
  // status sorting        
   ...
   if (sw_lateral_row[5] == '(E) Not Found' || sw_lateral_row[5] == '(E) Capped' )
   ...
   else if (sw_lateral_row[4].includes('Replace (E) as Directed') && sw_lateral_row[9] != 'Do Not Replace')
   
     {
    ...
    
      {
       ...
       if ( sw_lateral_row[6][0].includes('SW-23'))
   ...
   
   {
    ...
    return_block += '<td class="track_td ' + row_shade + '" style="text-align:center">' + sw_lateral_table[rr][2] +    '</td>';
