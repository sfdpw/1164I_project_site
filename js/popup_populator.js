...

        {
...

            {
...

                {
                    ...
                    output_string = output_string.concat("<a href=\".\\sheets\\1164I_dwg_", file_name, ".pdf\" target=\"_blank\">", (file_name.replace("-0", "-")).replace("_rev0", " Rev. "), "<\/a>");
...

{
    ...
    if (popup_args == null) {
...

        {
            ...
            {
                output_string = output_string.concat("<a href=\".\\specs\\1164I_", file_name, ".pdf\" target=\"_blank\">", file_name.substring(0, file_name_length), "<\/a> - ", file_name.substring(file_name_length+1, file_name.length).replace(/_/g,' '));
            } else

            {
                output_string = output_string.concat("<a href=\".\\specs\\1164I_", file_name, ".pdf\" target=\"_blank\">", file_name.substring(0, file_name_length).replace(/_/g,' '), "<\/a> - ", file_desc);
            }
...

    {
...

        {
            ...
            output_string += "<a href=\".\\pp_as_builts\\".concat(json_ab_list[ab_arg.substring(0, arg_length)][ii],
                ".pdf\" target=\"_blank\">", (json_ab_list[ab_arg.substring(0, arg_length)][ii].replace('1164I_', '')).replace(/_/g, ' ').replace('-0', '-'), "<\/a>");
